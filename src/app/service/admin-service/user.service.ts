...
export class UserService {
  ...
  getAllUserWithRoleName(): Observable<any> {
    return this.http.get(`${this.apiUrl}/allUsers`);
  }

  revokeUserRoleFromUser(userRoleId: number, userId: number): Observable<any> {
    return this.http.delete(`http://localhost:8083/api/userroles/revoke/${userRoleId}/${userId}`);
  }

  getAllUserWithAllRoleName(userId:number): Observable<any> {
    return this.http.get(`${this.apiUrl}/user/roleName/${userId}`);
  }

  getUserRolesWithUserId(userId:number): Observable<any> {
